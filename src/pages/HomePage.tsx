...
import { Car, CheckCircle2, Circle, Info, MapPin, CalendarClock, Pencil, PencilOff, Search, StopCircle, X, Check } from 'lucide-react';
  ...
  return (
    <span
      ...
      className={[
        ...
        editMode ? 'cursor-pointer underline decoration-dashed underline-offset-2 hover:opacity-70' : '',
      ]
        .filter(Boolean)
        .join(' ')}
    >
...
function VehicleCard({
  ...
  return (
    <div className={`space-y-2 py-4 rounded-lg transition-colors px-2 -mx-2 ${isActive ? 'bg-secondary/50' : ''}`}>
      ...
      <div className="pl-6 pt-2" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex flex-wrap gap-2">
          <Button size="sm" variant="outline" className="h-8 text-xs gap-1.5 cursor-pointer" onClick={onUpdateInfo}>
          ...
          <Button
            size="sm"
            variant="outline"
            className="h-8 text-xs gap-1.5 cursor-pointer border-red-500 text-red-500"
...
export function HomePage() {
  ...
  const [activeVehicleId, setActiveVehicleId] = useState<string | null>(MOCK_VEHICLES[0]?.id ?? null);
  ...
  const filteredVehicles = query
    ? vehicles.filter(
        v => v.unit.toLowerCase().includes(query) || v.plateNumber.toLowerCase().includes(query) || v.vin.toLowerCase().includes(query),
      )
    : vehicles;
  ...
  function handleUpdateInfoConfirm(info: VehicleInfo) {
    ...
    setVehicles(prev => prev.map(v => (v.id === updateInfoVehicleId ? { ...v, ...info } : v)));
    setUpdateInfoVehicleId(null);
  }

  function handleUpdateDate(id: string, field: 'registrationEndDate' | 'coiEndDate', value: string) {
      ...
      <Card>
        ...
        <CardContent>
          {filteredVehicles.length === 0 ? (
            <p className="text-muted-foreground text-sm">{search ? t('home.noVehiclesFound') : t('home.noVehicles')}</p>
          ) : (
